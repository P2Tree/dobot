...
int main(int argc, char * argv[])
{
    ...
    else if(!strcmp(argv[1], "RosSetPose")) {
...
void rosSetPoseCB(const dobot::DobotPose receivePose) {
    ROS_INFO("I heared: ( %02f, %02f, %02f, %02f )", receivePose.x, receivePose.y, receivePose.z, receivePose.r);
    DobotDriver dobot;
    Pose_t pose={1, 0.0, 0.0, 0.0, 0.0};
    pose.x = receivePose.x;
    pose.y = receivePose.y;
    pose.z = receivePose.z;
    pose.r = receivePose.r;
    int ret = dobot.runPointset(pose);
    if ( -1 == ret ) {
        perror("fault to run Pointset method to dobot");
    }
